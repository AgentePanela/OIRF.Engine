...
public sealed class FrameProfiler
{
    ...
    {
        Enabled = _cfg.Get(EngineCvars.ProfilerEnabled);
        GpuSyncEnabled = _cfg.Get(EngineCvars.ProfilerGpuSync);

        var window = Math.Max(2, _cfg.Get(EngineCvars.ProfilerWindow));
